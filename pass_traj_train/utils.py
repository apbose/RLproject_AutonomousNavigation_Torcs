...
#rollouts in the case of causal model
def rollout_causal_contin(env, agent, feat_extractor, feat_dim, aux_dim, encode_dim,
                   max_step_limit, min_step_limit, pre_step, paths_per_collect,
                   pre_actions, discriminate, posterior_causal):
    paths = []
    timesteps_sofar = 0
    encode_axis = 0
    encode_ini = np.zeros((1, encode_dim), dtype=np.float32)
    for p in xrange(paths_per_collect):
        print "Rollout index:", p
        feats, auxs, encodes, imgs, raws, actions, logstds = \
                [], [], [], [], [], [], []
        ...
        for i in xrange(max_step_limit):
            if i < min_step_limit:
                ...
                logstds.append(logstd)
                #we start with 0000
                action = agent.act(feat, aux, encode_ini, logstd)
                actions.append(action)
                imgs.append(img)
                raw = res[1]
                raws.append(raw)
                ...
                reward_d += discriminate.predict([img, aux, action])[0, 0] * 0.1
                encode_ini = posterior_causal.predict([img, aux, action, encode_ini])
