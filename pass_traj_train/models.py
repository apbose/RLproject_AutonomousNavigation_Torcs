...
#determines if we want the posterior network to be causal or not
parser.add_argument("--causal_option", type=int, default=0)
parser.add_argument("--paths_per_collect", type=int, default=10)
#the network are to be set and clamped with these values
parser.add_argument("--clamp_lower", type=float, default=-0.01)
parser.add_argument("--clamp_upper", type=float, default=0.01)
parser.add_argument("--lr_baseline", type=float, default=1e-4)
#mainly used for the drive trajectory, where initial presteps taken for 100, and then model is trained
...
#KL divergence in the TRPO case
...
#the discriminator and the baseline iterations, the baseline is used to reduce variance
...
#we sample random sample_size from the buffer and then train the model on the batch_size given
...
class TRPOAgent(object):
    ...
    def __init__(self, env, sess, feat_dim, aux_dim, encode_dim, action_dim,
        ...
        #KL divergence between the old and the new policy to be reduced
        self.oldaction_dist_mu = oldaction_dist_mu = tf.placeholder(dtype, shape=[None, action_dim])
        self.oldaction_dist_logstd = oldaction_dist_logstd = tf.placeholder(dtype, shape=[None, action_dim])
        #this option specifies if the causal posterior model is to be trained
        self.causal_option = args.causal_option


        # Build the network here
        print " TRPO generator is built"
        self.generator = self.create_generator(feats, auxs, encodes)
        print "Discriminator network is built"
        self.discriminator, self.discriminate = self.create_discriminator(img_dim, aux_dim, action_dim)
        print "Posterior network is built"
        if(not(self.causal_option)):
                  self.posterior = self.create_posterior(img_dim, aux_dim, action_dim, encode_dim)
                  self.posterior_target = self.create_posterior(img_dim, aux_dim, action_dim, encode_dim)
        elif(self.causal_option):
                  print "Now we build causal posterior"
                  self.posterior = self.create_causal_posterior(img_dim, aux_dim, action_dim, encode_dim)
                  self.posterior_target = self.create_causal_posterior(img_dim, aux_dim, action_dim, encode_dim)
    ...
    #use current state and previous latent variable c(t-1) to construct the latent code at t(p(ct)|st, ct-1)
    def create_causal_posterior(self, img_dim, aux_dim, action_dim, encode_dim):
        ...
        codeprev = Input(shape=[encode_dim])#we consider all the trajectories at once, so the encode_dim = 4
        h = merge([x, auxs, actions, codeprev], mode='concat')
        h = Dense(256)(h)
        h = LeakyReLU()(h)
        h = Dense(128)(h)
        h = LeakyReLU()(h)
        ...
        model = Model(input=[imgs, auxs, actions, codeprev], output=c)
    ...
    def learn(self, demo):
	    ...
	    if (not(self.causal_option)):
            ...
            rollouts = rollout_causal_contin(
                	self.env,
                	self,
                	self.feat_extractor,
                	self.feat_dim,
            ...
            for k in xrange(d_iter):
                #the discriminator will have dependance on actions and the latent code
                ...
                #actions_n would contain the policy network and the psoterior netwrok weights
            ...
            if(not(self.causal_option)):
            	...
            	for j in xrange(config.p_iter):
            	    ...
            	    for k in xrange(len(posterior_weights)):
            	        posterior_target_weights[k] = 0.5 * posterior_weights[k] + 0.5 * posterior_target_weights[k]
            ...
            path_idx = 0
            for path in paths:
                file_path = "//home/Apurba/Documents/RL/InfoGAIL/wgail_info_0/weights_causal/iter_%d_path_%d.txt" % (i, path_idx)
                f = open(file_path, "w")
                ...
                if(not(self.causal_option)):
                    output_p = self.posterior_target.predict(
                    [path["imgs"], path["auxs"], path["actions"]])
                    path["rewards"] = np.ones(path["raws"].shape[0]) * 2 + output_d.flatten() * 0.1 +  np.sum(np.log(output_p) * path["encodes"], axis=1)
                ...
                if(self.causal_option):
                    #calculating the baseline
                    path["rewards"] = np.ones(path["raws"].shape[0]) * 2 + output_d.flatten() * 0.1 
                    path_baselines = np.append(path["baselines"], 0 if
                                               path["baselines"].shape[0] == 100 else
                                               path["baselines"][-1])
                    deltas = path["rewards"] + config.gamma * path_baselines[1:] -\path_baselines[:-1]
                    #discount rewards over the different episodes in a single rollout
            ...
            param_dir = "/home/Apurba/Documents/RL/params_causal/"
...
#this serves the encoder layer for encoding the latent variables from the previous latent variables
class causal_posterior(object):
    def __init__(self, sess, img_dim, aux_dim, action_dim, encode_dim):
        self.sess = sess
        self.lr = tf.placeholder(tf.float32, shape=[])
        ...
        self.model = self.create_causal_posterior(img_dim, aux_dim, action_dim, encode_dim)
    ...
    def create_causal_posterior(self, img_dim, aux_dim, action_dim, encode_dim):
        ...
        auxs = Input(shape=[aux_dim])
        actions = Input(shape=[action_dim])
        codeprev = Input(shape=[encode_dim])
        #we consider all the trajectories at once, so the encode_dim = 4
        h = merge([x, auxs, actions, codeprev], mode='concat')
        h = Dense(256)(h)
        h = LeakyReLU()(h)
        h = Dense(128)(h)
